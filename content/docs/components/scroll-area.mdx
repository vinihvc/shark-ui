...
Use `scrollFade` to add a subtle fade effect to the edges of the scroll area. Ark also sets `--scroll-area-overflow-{x,y}-{start,end}` on the root if you want to drive a custom fade from those variables.
