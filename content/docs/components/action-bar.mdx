...
Update the `[--space]` CSS variable for internal padding.
...
Floating toolbar displayed at the bottom. Renders a `div`.
...
| `--offset` | `"16px"` |
| `--space` | Internal padding. Override via `className`, e.g. `[--space:--spacing(6)]`. Default: `--spacing(4)` |
