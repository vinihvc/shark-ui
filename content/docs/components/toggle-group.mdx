---
title: Toggle Group
description: A group of toggle buttons that can be used to select a value.
links:
  doc: https://ark-ui.com/docs/components/toggle-group
  api: https://ark-ui.com/docs/components/toggle-group#api-reference
---

<ComponentPreview componentName="toggle-group" />

## Installation

<CodeTabs>

<TabsList>
<TabsTrigger value="cli">CLI</TabsTrigger>
<TabsTrigger value="manual">Manual</TabsTrigger>
</TabsList>

<TabsContent value="cli">
  
```bash
npx shadcn@latest add @shark-ui/toggle-group
```

</TabsContent>

<TabsContent value="manual">

<Steps>

<Step>Install the following dependencies:</Step>

```bash
npm install @ark-ui/react tailwind-variants
```

<Step>Copy and paste the following code into your project.</Step>

<ComponentSource title="components/ui/toggle-group.tsx" src="/toggle-group.tsx" />

<Step>Update the import paths to match your project setup.</Step>

</Steps>

</TabsContent>

</CodeTabs>

## Usage

```tsx lineNumbers
import { 
  ToggleGroup, 
  ToggleGroupItem 
} from "@/components/ui/toggle-group"; 
```

```tsx lineNumbers
<ToggleGroup>
  <ToggleGroupItem value="1">Item 1</ToggleGroupItem>
  <ToggleGroupItem value="2">Item 2</ToggleGroupItem>
</ToggleGroup>
```

## Variants

### Outline

Use `variant="outline"` for an outline style.

<ComponentPreview componentName="toggle-group" fileName="example-outline" />

#### Ghost

Use `variant="ghost"` for a ghost style.

<ComponentPreview componentName="toggle-group" fileName="example-ghost" />

## Sizes

### Small

Use `size="sm"` for smaller toggles.

<ComponentPreview componentName="toggle-group" fileName="example-size-sm" />

### Large

Use `size="lg"` for larger toggles.

<ComponentPreview componentName="toggle-group" fileName="example-size-lg" />

## Examples

### Vertical

Use `orientation="vertical"` for a vertical layout.

<ComponentPreview componentName="toggle-group" fileName="example-vertical" />

### With spacing

Use `spacing` to add space between buttons. When spacing is greater than 0, each button keeps its own border and rounded corners.

<ComponentPreview componentName="toggle-group" fileName="example-with-spacing" />

### With separator

Add visual separators between toggle items using the Separator component.

<ComponentPreview componentName="toggle-group" fileName="example-with-separator" />

### Disabled

Disable the entire group with the `disabled` prop.

<ComponentPreview componentName="toggle-group" fileName="example-disabled" />

### With disabled toggle

Disable individual toggles by setting `disabled` on a specific item.

<ComponentPreview componentName="toggle-group" fileName="example-disabled-item" />

### Single selection

Use `multiple={false}` to allow only one toggle to be selected at a time.

<ComponentPreview componentName="toggle-group" fileName="example-single" />

### With tooltips

Wrap items with Tooltip to show labels on hover.

<ComponentPreview componentName="toggle-group" fileName="example-with-tooltips" />

### Custom

A custom toggle group example with tooltips and custom styling for font weight selection.

<ComponentPreview componentName="toggle-group" fileName="example-custom-font-weight" />

## API Reference

### ToggleGroup

Root component. Provides shared variant and size to child toggles via context.

| Prop | Type | Default | Description |
|------|------|---------|-------------|
| `variant` | `"outline"` \| `"ghost"` | `"ghost"` | Controls the styling of all child toggles |
| `size` | `"sm"` \| `"md"` \| `"lg"` | `"md"` | Controls the size of all child toggles |
| `spacing` | `0` \| `1` \| `2` \| `3` \| `4` \| `5` \| `6` \| `8` \| `10` | `0` | Gap between items. Use 0 for segmented (merged) look; &gt;0 keeps each button's border and radius |
| `multiple` | `boolean` | `true` | Whether multiple toggles can be selected |
| `defaultValue` | `string[]` | — | Initial selected value when uncontrolled |
| `value` | `string[]` | — | Controlled selected value |
| `onValueChange` | `(details: ValueChangeDetails) => void` | — | Callback when selection changes |
| `orientation` | `"horizontal"` \| `"vertical"` | `"horizontal"` | Layout direction of the group |
| `disabled` | `boolean` | `false` | Whether the group is disabled |

### ToggleGroupItem

Individual toggle button within the group. Inherits variant and size from parent ToggleGroup.

| Prop | Type | Default | Description |
|------|------|---------|-------------|
| `value` | `string` | **required** | Unique value for the item |
| `disabled` | `boolean` | `false` | Whether the item is disabled |