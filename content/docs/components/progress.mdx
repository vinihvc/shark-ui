---
title: Progress
description: Displays task progress with a linear indicator.
links:
...
Use `ProgressLabel` inside `Progress` to label the bar. `FieldLabel` as a sibling does not associate.
...
Accessible label. Render as a child of `Progress`.
