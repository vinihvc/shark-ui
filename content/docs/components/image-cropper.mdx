...
`useImageCropper` and `ImageCropperContext` expose `getCropData()` (natural-image `corners` and `outputSize`) and `getCroppedImage({ maxSize })` to cap the output while keeping aspect ratio.

<ComponentPreview componentName="image-cropper" fileName="example-export" />
