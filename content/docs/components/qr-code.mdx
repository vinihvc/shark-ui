...
Use `QrCodeOverlay` to add an overlay in the center. Downloads and `getDataUrl()` include the overlay in the exported image.
...
Use `QrCodeDownload` with `fileName` and `mimeType` to let users download the QR code as an image. The file includes any overlay placed on the code.
