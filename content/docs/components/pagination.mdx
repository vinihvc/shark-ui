...
`PaginationItemLink` renders an `<a>` whose `href` is `?page=`. Ark’s `type="link"` and `getPageUrl` set `href` on native First / Previous / Item / Next / Last parts. Shark wraps those parts in `Button`, so use `PaginationItemLink` for query-string links.
