...
<Step>This component depends on [Checkbox](/docs/components/checkbox). Install it first if you haven't already.</Step>
...
Icons are off by default. Pass icon components (and `showIndicator` for the expand chevron) when you want a file-tree look:
...
Use the `icon` and `expandedIcon` props on `TreeViewBranchItem` to customize folder icons. Omit them (or pass `null`) to hide. Set `showIndicator` to show the expand chevron.

<ComponentPreview componentName="tree-view" fileName="example-custom-icons-folder" />

### Single item icon

Pass the `icon` prop to `TreeViewItem` to show an icon for a specific leaf node.

<ComponentPreview componentName="tree-view" fileName="example-custom-icons-item" />

### With file extensions

Use the `fileIcons` prop on `TreeView` with `createFileIcons` to map file extensions to icon components. Icons are resolved from each node's id; unmatched extensions fall back to the `icon` prop on `TreeViewItem` (or no icon).
...
Clickable area to expand or collapse a branch. Icons and the expand chevron are off by default.

| Prop | Type | Default |
| --- | --- | --- |
| `asChild` | `boolean` | `false` |
| `showIndicator` | `boolean` | `false` |
| `icon` | `React.JSX.ElementType \| null` | `null` |
| `expandedIcon` | `React.JSX.ElementType \| null` | `null` |
| `className` | `string` | - |
...
Leaf node (non-expandable). Pass `icon` to show a leaf icon; `fileIcons` on `TreeView` wins when the node id matches an extension.
