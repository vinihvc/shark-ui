...
Use the `fileIcons` prop on `TreeView` with `createFileIcons` to map file extensions (e.g. `".tsx"`, `".json"`) to icon components. Icons are resolved from each node's id; unmatched extensions fall back to `FileIcon`.
...
Wrapper for leaf items. Use with `asChild` to render as a custom element (e.g. `<a>` for links). Renders a `div`.
