...
The color picker supports multiple usage modes: input for hex or channel editing, popover for area and slider selection, swatch picker for preset colors, and standalone area or slider components. Compose subcomponents to build the picker that fits your needs.
...
Use the `value` and `onValueChange` props to programmatically control the color picker's state. The exported `parseColor` helper parses color strings for programmatic use (e.g. `parseColor(value).toString("hex")`).
...
Container for the popover content. Renders inside a portal.
...
Checkerboard pattern for indicating transparency. Renders as a child of `ColorPickerSlider` when editing alpha.
...
Input for displaying and editing a color channel value. Use `asChild` with a custom input (e.g. `Input` or `InputGroupInput`) for styling.
...
Indicator shown when a swatch is selected. Renders a check icon by default; pass children to customize.
...
Exported utility to parse color strings. Use for programmatic color handling (e.g. `parseColor("#eb5e41").toString("hex")`).
