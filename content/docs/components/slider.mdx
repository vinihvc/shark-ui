...
Hold `Shift` or press `PageUp` / `PageDown` to move by `largeStep` (defaults to `10 * step`).

<ComponentPreview componentName="slider" fileName="example-large-step" />

### Thumb collision

On a range slider, `thumbCollisionBehavior` controls what happens when thumbs meet: `none`, `push`, or `swap`.
