...
description: Instructions to install Shark UI in a project manually.
---
...
```css title="styles/globals.css" lineNumbers
@import "tailwindcss";
...
@theme inline {
  ...
  --radius-lg: calc(var(--radius) * 1.125);
...
@layer base {
  ...
  * {
  ...
  body {
    @apply overflow-y-scroll bg-background text-foreground flex min-h-svh flex-col [--header-height:calc(var(--spacing)*14)] xl:[--footer-height:calc(var(--spacing)*24)];
  }
...
Now you can start adding [components](/docs/components) to your project.